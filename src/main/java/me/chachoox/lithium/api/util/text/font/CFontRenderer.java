...
public class CFontRenderer extends CFont {
    ...
    public void drawStringWithShadow(String text, double x, double y, int color) {
        double x2 = x;
        double y2 = y;
        if (!Managers.MODULE.get(CustomFont.class).shadow.getValue()) {
            x2 -= 0.4f;
            y2 -= 0.4f;
        }
        float shadowWidth = drawString(text, x2 + 1.0D, y2 + 1.0D, color, true);
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow) {
        ...
        //TODO: WHERE TF DID I FUCK UP THIS
        for (int i = 0; i < size; i++) {
            ...
            if (character == '\u00a7') {
                ...
                if (rainbowP || rainbowM || rainbowE) {
                    int rainbow = HSLColor.toRGB(Colours.get().getRainbowHueByPosition(getRainbowPos(rainbowP, rainbowM, rainbowE, x, y) * Colours.get().factor.getValue()), Colours.get().saturation.getValue(), Colours.get().lightness.getValue()).getRGB();
    ...
    private void setupBoldItalicIDs() {
        texBold =
                setupTexture(
                        this.font.deriveFont(Font.BOLD), this.antiAlias, this.fractionalMetrics, this.boldChars);
        texItalic =
                setupTexture(
                        this.font.deriveFont(Font.BOLD), this.antiAlias, this.fractionalMetrics, this.italicChars);
        texItalicBold =
                setupTexture(
                        this.font.deriveFont(Font.BOLD | Font.ITALIC), this.antiAlias, this.fractionalMetrics, this.boldItalicChars);
    }
